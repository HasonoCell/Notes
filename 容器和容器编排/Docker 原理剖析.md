...
所以 **mount** 这个命令的本质，就是指就是将一个实现了 VFS 标准接口的具体文件系统实例（即它的 `super_block` 和根 `inode`），绑定到 VFS 全局目录树中的某一个特定的 `dentry` 节点（即挂载点）上。比如将一个 Ext4 fs 的 U 盘插入到 linux os 的主机上，如果这个主机的 linux 环境是最纯净的 linux 环境，没有图形界面，也没有任何的自动化服务，当插入 U 盘核在 `/dev` 目录（设备文件目录）下生成 U 盘的文件节点 `/dev/sdb1` ，但此时 VFS 还不能找到访问到 U 盘中存储的内容，必须**手动执行 mount 系统调用：`mount -t ext4 /dev/sdb1 /mnt/usb`**，当访问 `/mnt/usb` 时，会被 VFS 引导到该进程的 mount namespace 中的挂载表去。表中有 **`/mnt/usb -> U盘文件系统的根 dentry`** 这样一条记录。从而让内核**跨越到 U 盘专属的 dentry 树中继续向下解析**。当最终找到目标文件的 inode 需要读写磁盘时，才会通过所属的 super_block 找到并操作底层的 `/dev/sdb1` 块设备。

如果是**绑定挂载**，同一个文件系统的目录挂载目录，其实就是通过该进程命名空间的挂载表，修改访问的 dentry，进而也就实际修改了访问的 inode 啦。
...
1. **实例分配**：内核调用 `kmalloc`，在内存中分配一个新的 `struct mnt_namespace` 结构体实例。
2. **挂载表拷贝**：内核会遍历父进程当前所指向的挂载表，将里面现有的所有映射记录（即 `struct mount` 对象）在内存中完整地拷贝一份，填充到刚刚分配的新 `mnt_namespace` 实例中。
3. **重定向代理指针**：内核将新进程的 `task_struct -> nsproxy -> mnt_ns` 指针，从原来指向全局的实例，更新为指向这个刚刚创建的全新 `mnt_namespace` 实例。

现在，mount namespace 中的新进程会完整拷贝父进程的挂载表。也就是说，新进程虽然有了全新的 `mnt_namespace` ，但内容还是和父进程完全相同，比如新进程访问 `/` 路径，看到的还是宿主机的根路径。在现代基于 systemd 的 Linux 中，根目录 `/` 默认具有 **Shared（共享）属性**。拷贝过来的挂载表页会继承这一属性。这意味着，在切换到独立的镜像根目录之前，存在一个过渡期：如果新进程在这个时期直接进行 mount 操作（例如准备新根或挂载 /proc），这些操作会顺着 Shared 属性的隐形通道传播（Propagate）回宿主机，污染甚至摧毁宿主机的文件系统。因此，新进程启动后必须立刻执行一个关键的系统调用 `mount("none", "/", "", MS_REC | MS_PRIVATE, "")`，`MS_PRIVATE` 将当前 Namespace 的挂载属性改为私有，彻底切断与宿主机的共享机制。`MS_REC` 递归应用，不仅 `/` 变成私有，底下的所有已存在的子挂载点也全部变为私有。也就是说，这两个 flags 主要目的就是用来**在容器内进程更新根挂载点前，切断与宿主机的共享机制的**，确保了宿主机的文件系统绝对不会被容器内进程覆写更改。

前面我们说了，mount namespace 中新进程的挂载表内容在初期仍然和宿主机相同。比如想要运行一个 Ubuntu 的**容器**，此时容器引擎虽然把一个包含 Ubuntu 文件的**镜像**解压到了宿主机的某个目录（例如 `/var/lib/docker/overlay2/xxx`），但在新进程的视角里，当它执行 `open("/bin/bash")` 时，VFS 依然会顺着它私有挂载表中的 `/` 记录，去 `/dev/sda1`（宿主机的盘）里找 `/bin/bash`。为了让容器进程认为自己运行在 Ubuntu 中，我们必须把该新进程 `mnt_namespace` 中对 `/` 的映射记录，强行修改为指向那个包含 Ubuntu 文件的镜像解压目录。
...
然后新进程调用 `pivot_root("/new_root", "/new_root/put_old")`。内核的 VFS 介入，直接操作当前进程的 `mnt_namespace` 挂载表，将原本映射到 `OverlayFS` 的记录 B，其挂载点强制修改为 `/`；将原本映射到宿主机磁盘的记录 A，其挂载点强制修改为 `/put_old`。执行完 `pivot_root` 后，容器进程私有挂载表的状态：

- 记录 B 变成了：`/` -> 容器镜像设备 (`OverlayFS`)
- 记录 A 变成了：`/put_old` -> 宿主机磁盘 (`/dev/sda1`)
    

当容器进程再次执行 `open("/bin/bash")` 时，VFS 从当前 `mnt_namespace` 的 `/` 开始解析，发现其映射到了 `OverlayFS`。于是，请求被路由到了 Ubuntu 镜像中。进程成功读取到了 Ubuntu 的 `bash` 文件。为了防止容器进程通过访问 `/put_old` 目录依然能看到宿主机的文件，容器引擎会紧接着执行最后一步： `umount("/put_old", MNT_DETACH)` 这会将包含宿主机文件系统的映射记录 A 从该 `mnt_namespace` 中彻底删除。并且，由于前面的两个 Flags 切断了传播机制，在新进程的 mount 和 umount 操作后，宿主机的文件系统也不会受到任何影响，这也再次强调了 **namespace 只决定这其中的进程能看到什么样的资源，但是不会确保进程对资源的操作会不会影响到宿主机**，比如在下面的 uts namespace 中新进程 `sethostname` 修改主机名就不会影响宿主机，因为 `struct uts_namespace` 是个极其简单的结构体直接拷贝即可，但是 mount namespace 由于文件系统的传播机制存在，所以导致了命名空间中的进程对文件资源的修改在宿主机中也会受到影响。至此，容器进程的挂载表中，再也没有任何一条路径可以路由到宿主机的底层设备上。文件系统的 VFS 隔离彻底完成。
...
这里还有一个很有意思的的点就是，如果你想通过 `rm -rf` 命令删除一个 cgroups，比如 my_container 是删除不掉的，这是因为这个命令本质上是进入目录然后对目录中的每个文件调用 `unlink` 系统调用，但是我们前面说了，cgroups 中的文件并不存在磁盘实体，本质上只是 kernel 中存储在内存上的数据结构，自然无法 `unlink`。这时候我们可以通过 `rmdir` 这个命令来释放 cgroup struct。

我们还可以按照下面的步骤手动创建一个 cgroups 来玩玩：

```bash

# 创建一个 cgroups，内核分配 struct cgroup 实例并将其挂接到统一层级树上。此时该目录下会自动生成 cpu.max、cgroup.procs 等接口文件。
mkdir /sys/fs/cgroup/my_sandbox

# 内核将 cfs_bandwidth 结构体中的 quota 修改为 50ms，period 修改为 100ms。
echo "50000 100000" > /sys/fs/cgroup/my_sandbox/cpu.max

# 在一个新终端运行下面这条死循环，该进程所在的默认 cgroup 会让它吃满 100% 的一个 CPU 核心，你可以用top 命令查看其资源占用和 pid
while :; do :; done

# 更改该进程的默认 cgroup 为刚刚创建的 my_sandbox，再通过 top 目录查看该进程 CPU 资源占额，发现只有 50% 了
echo <pid> > /sys/fs/cgroup/my_sandbox/cgroup.procs
```
...
我们可以先手动通过系统调用打造一个 OverlayFS 挂载点来体验一下：
...
# 在“只读镜像层”写入一个初始配置
echo "version=1.0" > /tmp/overlay_demo/lower/config.txt

# 执行完整挂载命令
sudo mount -t overlay overlay -o lowerdir=/tmp/overlay_demo/lower,upperdir=/tmp/overlay_demo/upper,workdir=/tmp/overlay_demo/work /tmp/overlay_demo/merged 
# merged 将作为容器内进程的根挂载点（通过 mount namespace 来实现，其底层通过 pivot_root 来改变根挂载点），容器内进程在它的根挂载点的所有操作都会被 VFS 分发给 OverlayFS，再由 OverlayFS 根据命令读写 upper 或者读 lower。我们这里不会真正启动容器，只是理一下思路。

# 1. 验证合并视图：进程从 merged 挂载点成功读到了 lower 的数据
cat /tmp/overlay_demo/merged/config.txt
# 输出应该为: version=1.0

# 2. 触发修改 (写操作)：进程通过 merged 挂载点修改文件
echo "version=2.0 (modified)" > /tmp/overlay_demo/merged/config.txt

# 3. 验证物理隔离：查看底层物理目录的真实状态
cat /tmp/overlay_demo/lower/config.txt
# 输出依然是: version=1.0 (底层镜像被严格保护，绝对只读)

cat /tmp/overlay_demo/upper/config.txt
# 输出变为: version=2.0 (modified) (触发了 Copy-Up，数据被物理落盘在了 upperdir)
```

所以从上面的例子我们可以发现，容器引擎准备容器的文件系统时，会先准备好 OverlayFS，再通过 Mount Namespace 更改根挂载点。

# 容器必要的 Network 知识

namespace 对于容器一个至关重要的 ns 就是 netns。为容器进程创建一个新的 netns，本质上是在新的 netns 中创建一个新的 linux 网络栈并设置该网络栈。

**网卡**：电脑、手机等设备接入网络的硬件设备。把电脑数字信号转成光/电信号发出去，把接收到的信号转成电脑能理解的二进制数据。物理网卡的一端连接着网线与外部沟通，一端和 kernel 的网络栈连接，数据包通过网线传到网卡，网卡再将数据包转发给网络栈处理。

**MAC（Media Access Control Address） 地址**：网卡在出厂时就烧录在固件里的一个唯一地址。它用来在一个局域网内唯一标示一台设备，帮助数据包在一个局域网内定位设备。即使换了网络，MAC 地址通常也不变。

**IP（Internet Protocol Address）地址**：用来确定到底属于哪一个网络。用 IP 定位网络之后，就可以再通过 MAC 地址，去找到数据包到底该发给哪台设备。IP 地址由**网络号和主机号**两部分组成，网络号用来定位一个具体的子网，主机号用来在该子网中定位具体的设备。

**路由机和交换机**：**路由机**连接不同的网络，根据目标 IP 地址决定数据包走哪条路传输。**交换机**用来识别 IP 地址中的主机号部分，通过 **ARP 协议**将其转换为目标设备的 MAC 地址，从而将数据包转发给最终的目标设备，这就是为什么光有 IP 地址主机号还不够，因为交换机是一个物理层设备，有了 MAC 地址才能找到目标设备。

## 容器间通信原理

假设一台物理机上没有运行任何容器和虚拟机，那么这个物理机上的所有进程都是用的 linux 系统默认 nets 中的网络栈，IP，MAC 等等信息都是一样的。这时候如果创建了个新的 netns，即重新创建一个全新的网络栈，那么这个 netns 中的 IP，MAC 等等信息都和宿主机默认 netns 的网络栈信息都不同了，这时候如果有进程加入到了这个新的 netns，我们甚至可以说这个进程就相当于是一个“有独立网络环境的新设备”，那它和宿主机之间怎么通信呢？传统的无线网卡（WIFI）或者有线网卡（网线）都起不了作用，这时候就需要一个虚拟网卡 veth，用来在新的 netns 和宿主机的默认 netns 中“拉网线”，建立一个点对点通信，才能传输数据包实现通信。
...
前面说了容器之间以及容器和宿主机如何通信，那如何让容器通过宿主机与公网通信呢？通过端口映射（比如 `-p 8080:80`），即 **DNAT (Destination Network Address Translation，目标网络地址转换)** 来实现，DNAT 的底层是通过 Linux 的 Netfilter 子系统（通常使用 `iptables` 命令来管理）实现的

当一个来自公网的请求想要访问一台物理机上的容器服务时，假设宿主机上的一个 linux bridge 的 IP 地址是 `10.200.0.2`，数据包的完整经历是这样的：

1. 发起请求，目标是宿主机的公网 IP 和 8080 端口。数据包通过真实的网线，到达了宿主机的物理网卡（比如 `eth0`）。
2. 数据包刚进入宿主机的网络栈，立马被 `iptables` 设置的 `PREROUTING`（路由前）规则拦截了。
3.  `iptables` 规则检查发现这个请求的目标 IP 地址和端口号应该被转换，目标 IP 换成 `10.200.0.2`，目标端口换成 `80`。kernel 发现这个网段由 linux bridge 管理。数据包被丢给了 bridge，bridge 通过之前配置好的 `veth pair` ，把数据包送进容器的 netns。如果容器中的进程正在监听该 netns 虚拟网卡的 80 端口，就可以正常处理请求。

这篇博客 [Linux 虚拟化之：Net Namespace 下的网络通信 \| 国南之境](https://hansedong.github.io/2018/12/21/12/) 讲的挺不错的，可以参考一下
...
我们知道容器镜像并不是一个真正的操作系统，它的本质是一个打包了应用程序及其完整运行时环境的、结构化的静态 **“文件系统快照”**。镜像不包含操作系统的内核（Kernel），它完全共享宿主机的内核。镜像内部只包含内核之上的用户空间内容，例如 `/bin`、`/etc` 等目录，以及 `glibc` 库、系统工具（`bash`/`apt` 等）和应用程序。

镜像在磁盘上并不是一个单一的巨型文件，而是由多个**只读层（ReadOnly Layers）** 叠加而成的。Docker 等容器运行时利用 OverlayFS 技术将它们整合。
* **只读层（lowerdir）**：镜像的每一层都是一个独立的、不可变的文件压缩包。它们像多层抽屉一样重叠在一起。
* **可写层（upperdir）**：当容器启动时，容器运行时会在镜像最顶层挂载一个极薄的、可读写的临时层。
* **统一视角（mergedir）**：在容器内部的进程看来，这些层被“联合挂载”成了一个普通的、连续的、完整的 Linux 根文件系统（rootfs）。

为了保证高效利用磁盘和内存，容器在读写镜像文件时采用了 **Copy-on-Write（写时复制）** 机制。
* **读文件**：如果容器读取的文件只存在于底层的只读层，容器会直接读取该文件，没有任何性能损耗。
* **修改文件**：如果容器尝试修改某个只读层的文件，OverlayFS 会先将该文件复制到顶层的“可写层（upperdir）”，然后对副本进行修改。
* **核心优势**：这种设计使得同一个镜像可以被成百上千个容器同时共享，每个容器只需占用极小的磁盘空间来存放自己独有的改动。

从分发和标准的角度来看，符合 OCI 规范的镜像，其物理本质其实非常朴素。
* **Tar 包的集合**：镜像的每一层在物理存储上就是一个个打包并压缩好的 `.tar.gz` 文件（在镜像仓库中被称为 Blob）。
* **清单与配置**：除了 Tar 包，镜像还包含一个或多个 JSON 格式的配置文件。这些 JSON 文件记录了镜像的层级顺序、每层的哈希值（SHA256）、环境变量、启动命令（ENTRYPOINT）等元数据。
