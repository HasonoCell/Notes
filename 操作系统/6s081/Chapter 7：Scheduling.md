...
2. 旧进程通过 yield/sleep 等路径调用 swtch，切到调度器 scheduler，把当前寄存器现场（主要是 sp 和 ra 两个寄存器，sp 为栈顶指针，指向函数调用栈的栈顶位置的内存，而 ra 用来保存一个函数调用结束后该回到哪里）保存到 proc->context。也就是说 **CPU 是直接操作其寄存器的，切换时，将寄存器的值保存到内存，又将立马要计算的值从内存恢复到寄存器**。
3. scheduler 在全局进程表（本质是 struct proc 数组）中扫描，选中一个 RUNNABLE 进程，再通过 swtch 恢复它之前保存的内核上下文。
4. 新进程随后通过 trap return 回到用户态，继续执行用户程序。

![](assets/Chapter%207：Scheduling/file-20260519163253320.png)
