...
当构造完 `Form-Data` 对象后，就可以通过网络进行数据交互了。我们知道，网络中传输的数据需要遵守对应的HTTP 数据传输格式规范，比如你想通过网络传输一个 `JSON` 数据，那就需要遵循 `application/json`这一数据传输格式规范`(JSON.stringify(data))`。同样，你想通过网络传输一个 `Form-Data`数据，就需要遵循 multipart/form-data 这一格式规范。通常，</font>`<font style="color:rgb(64, 64, 64);background-color:rgb(236, 236, 236);">Form-Data</font>`<font style="color:rgb(64, 64, 64);"> 对象会自动以 </font>`<font style="color:rgb(64, 64, 64);background-color:rgb(236, 236, 236);">multipart/form-data</font>`<font style="color:rgb(64, 64, 64);"> 格式编码数据。（前端注意设置</font>`<font style="color:rgb(64, 64, 64);">Content-Type: multipart/form-data</font>`<font style="color:rgb(64, 64, 64);">哦）</font>
