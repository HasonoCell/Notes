...
当构造完 `Form-Data` 对象后，就可以通过网络进行数据交互了。我们知道，**网络中传输的数据需要遵守对应的****<font style="color:rgb(64, 64, 64);">HTTP 数据传输格式规范</font>**，比如你想通过网络传输一个 `JSON` 数据，那就需要遵循 `application/json`这一数据传输格式规范`(JSON.stringify(data))`。同样，你想通过网络传输一个 `Form-Data`数据，就需要遵循`<font style="color:rgb(64, 64, 64);">multipart/form-data</font>`<font style="color:rgb(64, 64, 64);">这一格式规范。通常，</font>`<font style="color:rgb(64, 64, 64);background-color:rgb(236, 236, 236);">Form-Data</font>`<font style="color:rgb(64, 64, 64);"> 对象会自动以 </font>`<font style="color:rgb(64, 64, 64);background-color:rgb(236, 236, 236);">multipart/form-data</font>`<font style="color:rgb(64, 64, 64);"> 格式编码数据。（前端注意设置</font>`<font style="color:rgb(64, 64, 64);">Content-Type: multipart/form-data</font>`<font style="color:rgb(64, 64, 64);">哦）</font>
...
`<font style="color:rgb(64, 64, 64);">boundary</font>`<font style="color:rgb(64, 64, 64);">：随机生成的边界分隔符（浏览器自动处理）</font>

`<font style="color:rgb(64, 64, 64);">Content-Disposition</font>`<font style="color:rgb(64, 64, 64);">：包含字段名/文件名</font>

`<font style="color:rgb(64, 64, 64);">Content-Type</font>`<font style="color:rgb(64, 64, 64);">：文件类型（可选）</font>
