...
namespace 对于容器一个至关重要的 ns 就是 netns。为容器进程创建一个新的 netns，本质上是在新的 netns 中创建一个新的 linux 网络栈并设置网络栈。
...
**IP（Internet Protocol Address）地址**：用来确定到底属于哪一个网络。用 IP 定位网络之后，就可以再通过 MAC 地址，去找到数据包到底该发给哪台设备。

**路由机和交换机**：**路由机**连接不同的网络，根据目标 IP 地址决定数据包走哪条路传输。**交换机**用来把一堆设备连成一个局域网，识别 MAC 地址从而将数据包转发给最终的目标设备。路由机对一个网络外处理，交换机对一个网络内处理。

在创建 net namespace 之后，子进程就彻底隔离了宿主机的网卡，IP，MAC，路由等信息（隔离一整套网络栈）。veth pair 相当于一根虚拟网线，用来联通宿主机和命名空间内进程，数据包就在 veth pair 中传输。Linux Bridge 相当于一个虚拟交换机，用来在多个虚拟机/容器转发数据包。如果有多个容器，每个容器都有一对 veth，每对 veth 的 host 端都接到同一个 bridge 上，那这些容器就像都插到了同一个交换机上，于是它们就可以互相通信。所以，容器引擎要做的就是**给每个容器单独开一个网络栈，再用一根虚拟网线把它接到 Linux Bridge 上，并给它配置同一局域网里的 IP 和路由**

此外，这篇博客 [Linux 虚拟化之：Net Namespace 下的网络通信 \| 国南之境](https://hansedong.github.io/2018/12/21/12/) 讲的挺不错的，可以参考一下