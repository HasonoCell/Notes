...
举个例子，比如用户访问 www.baidu.com ，这是个域名，首先由 DNS 系统解析为 IP 地址，然后用户就有了目标服务器的 IP 地址，接着发送一个网络请求给服务器，这个请求有极大概率就会先被打到 Nginx 上（跑在目标服务器的 80 端口）

- 如果访问静态资源，直接秒拿。
- 而如果对 API 接口发送了请求，Nginx 再做反向代理，将请求打到用 Java/GO 写的后端服务里面去（比如说有可能跑在 3000 端口上面），这个后端服务再去处理逻辑，读写数据库，返回数据。
...
在开发机上，将 React/Vue 代码编译成浏览器能读懂的静态文件。
...
学习笔记直接参考：[Docker 简易入门](Docker%20简易入门.md)