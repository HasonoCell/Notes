...
// PX 这个宏表示从虚拟地址 va 里，取出某一级页表要用的那 9 个 bit
...
所以，一张页表在 xv6 中并不是一次性创建完成的，而是逐步“生长”出来的～
...
1. 调用 kalloc() 分配一页物理内存
2. 把这一页清零
3. 把这一页作为新的根页表页返回

这里最关键的一点是：uvmcreate() 创建出来的，并不是整张完整页表，而只是根页表页。

此时这张根页表页里虽然已经有了 512 个 PTE 槽位，但它们全都是 0，也就是说：

- 还没有任何合法映射
- 也还没有任何下一层页表页被挂上来

所以这时候的页表，严格来说只是“一棵空树的根”。
