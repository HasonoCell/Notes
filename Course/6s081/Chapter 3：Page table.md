...
如果只从教材里看 “page table” 这个词，很容易误以为页表是一整张巨大的表，里面直接存满了从虚拟地址到物理地址的映射。但在 xv6 的实现里，页表并不是一个单独的大数组，而
是一个由多个页表页（page-table page）组成的三级树结构。
...
为什么 pte_t 要用 uint64 来表示？因为在 RISC-V 的 Sv39 机制里，一个页表项本来就是一个 64 位宽的数据结构，里面既要保存物理页号 PPN，也要保存一组标志位 flags，例如：
...
所以 PTE 本质上不是一个普通整数，而是一个“按位切分的 64 位盒子”，其中高位存的是物理页号，低位存的是权限位和状态位。

不过，pagetable_t 这个名字一开始很容易让人误解。它虽然叫 “page table”，但从代码实现上看，它其实更准确地说是：根页表页的起始地址。也就是说：

- 在抽象意义上，pagetable_t 代表“一整张页表”
- 在实现意义上，它存的其实只是“根页表页”的地址

为什么说它是“根页表页”的地址，而不是“整棵树本身”？这就要直接看代码了。

在 kernel/vm.c 的 walk() 函数中，有这样一句非常关键的代码：

pte_t *pte = &pagetable[PX(level, va)];

这一句几乎把 pagetable_t 的本质直接暴露出来了。因为只有当 pagetable 是“数组起始地址”时，我们才能像这样写：

pagetable[index]

这说明，pagetable 在 C 语言里被当成了一个数组来使用。而这个数组中的每一个元素，都是一个 PTE。换句话说，一张页表页在 C 语言眼中，其实就是这样一种东西：

pagetable = [ PTE0, PTE1, PTE2, ..., PTE511 ]

也就是说，一张页表页，本质上就是一个由 512 个 PTE 组成的数组。

为什么正好是 512 个？因为：

- xv6 中一页大小是 4KB = 4096 字节
- 一个 PTE 的大小是 8 字节，也就是一个 uint64

所以：

4096 / 8 = 512

这意味着，一个页表页刚好可以存放 512 个页表项。而 Sv39 虚拟地址中每一级索引正好是 9 位，这也正好对应：
...
到这里，我们已经可以对“页表在 xv6 中如何定义”有一个更清晰的认识了：

1. pte_t 是一个 64 位页表项
2. pagetable_t 是一个指向页表页起始位置的指针
3. 一张页表页本质上是一个 PTE[512] 数组

不过这里还要再强调一个非常重要的点：一张页表并不等于一张页表页。

在 Sv39 里，真正的一整张页表是一个三级结构。也就是说：
