...
当用户态 trap 发生时，CPU 会跳到寄存器 stvec 指向的物理地址去执行，而对于用户态 trap，stvec 被设置为 trampoline page 中的 uservec。所以，stvec 寄存器可以看作是 trap 发生后整个处理流程的入口，从 stvec 找到切换代码（即 TRAMPOLINE 对应的物理地址处保存的汇编代码）。
