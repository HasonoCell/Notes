...
关于如何避免**死锁**，xv6 没有复杂的调度算法，实现方式是要求代码路径一致，比如有两个锁 A 和 B，路径 T1 是先 A 后 B，T2 是先 B 后 A，这样就会造成死锁，所以一种避免的方式是强制所有路径要么是 T1，要么是 T2。xv6 通过固定的代码路径和锁嵌套关系，形成了隐式的锁顺序约束。比如拥有最长**锁链**的文件系统：
