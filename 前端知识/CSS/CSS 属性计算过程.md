...
![](https://cdn.nlark.com/yuque/0/2025/png/53866714/1752136595756-b7d5d084-696e-49c4-bd1f-c85595f507e8.png)
...
更详细的来源重要性比较，可以参阅 _MDN_：[https://developer.mozilla.org/zh-CN/docs/Web/CSS/Cascade](https://developer.mozilla.org/zh-CN/docs/Web/CSS/Cascade)



我们来看一个示例。

例如现在有**页面作者样式表**和**用户代理样式表**中存在属性的冲突，那么会以页面作者样式表优先。
...
可以明显的看到，页面作者样式表和用户代理样式表中同时存在的 display 属性的设置，最终作者样式表干掉了用户代理样式表中冲突的属性。这就是第一步，根据不同源的重要性来决定应用哪一个源的样式。
...
![](https://cdn.nlark.com/yuque/0/2025/png/53866714/1752136595796-338bbe16-41f5-42fd-8953-46fac6b5aa7e.png)

可以看到，落败的作者样式在 _Elements>Styles_ 中会被划掉。

有关选择器权重的计算方式，不清楚的同学，可以进入此传送门：[https://developer.mozilla.org/en-US/docs/Web/CSS/Specificity](https://developer.mozilla.org/en-US/docs/Web/CSS/Specificity)
...
## 使用默认值
好了，目前走到这一步，如果属性值都还不能确定下来，那么就只能是使用默认值了。这里的默认值是规定在浏览器的源码中的，如 Chrome 的 C++ 源码中，不做过多叙述。
